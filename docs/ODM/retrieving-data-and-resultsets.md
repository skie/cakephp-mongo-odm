...
> `docs/orm/retrieving-data-and-resultsets.md` in full. Getting a single
> document, the finder system (all/list/threaded + custom + dynamic finders),
> `unhydratedFind()`, eager loading via `contain()`, filtering via
> `matching()`/`notMatching()`/`innerJoinWith()`/`leftJoinWith()`, ResultSets,
> `loadInto()` and Map/Reduce are all covered here.
>
...
> (`SelectQueryTest`, `EagerLoaderTest`, `ResultSetTest`, `BaseCollectionTest`).
> Before the `### PENDING` banner is removed, the remaining samples must be run
> against a live test database (see
> `docs/working-memory-docs/49-retrieving-data-analysis.md`).

`class` Crustum\Mongo\ODM\**BaseCollection**

While collection objects provide an abstraction around a 'repository' or
collection of objects, when you query for individual records you get 'document'
objects. While this section discusses the different ways you can find and load
documents, you should read the [Documents](./documents) section for more
information on documents.

## Debugging Queries and ResultSets

Since the ODM now returns Collections and Documents, debugging these objects can
be more complicated than in previous versions. There are now various ways to
inspect the data returned by the ODM.

- `debug($query)` Shows the compiled Mongo filter/pipeline and options, does not show results.
- `sql($query)` Shows the compiled query in a Mongo-readable form (filter, projection, sort, pipeline stages).
...
- `debug($query->first())` Show the properties of a single document.
- `debug((string)$query->first())` Show the properties of a single document as JSON.

## Getting a Single Document by Primary Key

`method` Crustum\Mongo\ODM\BaseCollection::**get**(mixed $primaryKey, array|string $finder = 'all', CacheInterface|string|null $cache = null, Closure|string|null $cacheKey = null, mixed ...$args): DocumentInterface

It is often convenient to load a single document from the database when editing
or viewing documents and their related data. You can do this by using `get()`:
...
either catch this exception yourself, or allow your application to convert it
into a 404 error.
...
`method` Crustum\Mongo\ODM\BaseCollection::**find**(string $type = 'all', mixed ...$args): SelectQuery

Before you can work with documents, you'll need to load them. The easiest way to
...
a `Crustum\Mongo\ODM\Query\SelectQuery` object. The SelectQuery class allows you
to further refine a query after creating it. SelectQuery objects are evaluated
lazily, and do not execute until you start fetching rows, convert it to an
array, or when the `all()` method is called:
...
> [!NOTE]
> Once you've started a query you can use the [Query Builder](./query-builder)
...
- `conditions` provide conditions for the filter of your query.
...
- `fields` limit the fields loaded into the document. Only loading some fields
  can cause documents to behave incorrectly.
- `group` add a `$group` stage to your query. This is useful when using
  aggregating functions.
- `having` add a `$match` stage on the grouped results to your query.
- `order` order the result set.

> [!NOTE]
> The SQL `join` named option has no direct Mongo analog. To filter or load data
> through associations use `contain()`, `matching()`, `innerJoinWith()` or
> `leftJoinWith()` instead (see below).
...
### Getting Arrays Instead of Documents

By default, queries and result sets return [Documents](./documents) objects.
...
It returns a `Crustum\Mongo\ODM\Query\UnhydratedSelectQuery`. This behaves
exactly like `find()->hydrate(false)` at runtime, but its static type matches
the array result shape, so static analyzers no longer see `entity|array` on
`first()`, `firstOrFail()`, `all()`, `toArray()` and iteration — including after
the query flows through a custom finder.
...
> [!TIP]
> Prefer `unhydratedFind()` over `hydrate(false)` when your code only ever
> needs arrays: the returned query is statically typed as producing arrays, so
> you keep full static-analysis coverage.
...
This approach replaces `find('first')` in previous versions. You may also want
to use the `get()` method if you are loading documents by primary key.
...
The count is cached until the query is modified. When the query carries an
in-pipeline eager load (`matching()` / `contain()` via `$lookup`), grouping or
`having`, the count runs through the aggregation pipeline so joined and
filtered rows are counted correctly; otherwise a fast collection-level count is
used.

See [Query Count](./query-builder#query-count) for additional usage of the `count()` method.
...
elements. The ODM provides a simple to use method for generating 'lists' of
data:
...
// Data now looks like
$data = [
    '000000000000000000000001' => 'First post',
    '000000000000000000000002' => 'Second article I wrote',
];
```

With no additional options the keys of `$data` will be the primary key of your
collection, while the values will be the 'displayField' of the collection. The
default ‘displayField’ of the collection is `title` or `name`. While, you can
use the `setDisplayField()` method on a collection object to configure the
display field of a collection:
...
You can also create list data from associations:
...
document attribute paths not the database fields. This means that you can use
virtual fields in the results of `find(list)`.

### Customize Key-Value Output

Lastly it is possible to use closures to access document accessor methods in
your list finds.

```php
// In your Author document create a virtual field to be used as the displayField:
...
```php
// In AuthorsCollection::initialize():
$this->setDisplayField('label'); // Will utilize Author::_getLabel()
// In your finders/controller:
$query = $authors->find('list'); // Will utilize AuthorsCollection::getDisplayField()
```

## Finding Threaded Data

The `find('threaded')` finder returns nested documents that are threaded
...
collection. All documents matching a given `parent_id` are placed under the
`children` attribute:
...
$query = $comments->find('threaded',
    keyField: $comments->getPrimaryKey(),
...
wanted to add a finder to our articles collection for finding articles written
by a given user, we would do the following:
...
While all the examples so far have shown finder methods on collection classes,
finder methods can also be defined on [Behaviors](./behaviors).

If you need to modify the results after they have been fetched you should use
a [Map Reduce](#map-reduce) function to modify the results. The map reduce
features replace the 'afterFind' callback found in previous versions.

## Dynamic Finders

The ODM provides dynamically constructed finder methods which allow you to
...
>
> A field named `id` in a dynamic finder maps to the Mongo `_id` primary key
> automatically (`findById` → `where(['_id' => ...])`).
...
- use ODM query functions like `contain()` and `matching()`
- use join functions like `innerJoinWith()` and `leftJoinWith()`

You should use `contain()` when you want to load the primary collection, and its
associated data. While `contain()` will let you apply additional conditions to
the loaded associations, you cannot constrain the primary collection based on
the associations. For more details on the `contain()`, look at
[Eager Loading Associations](#eager-loading-associations).

You should use `matching()` when you want to restrict the primary collection
based on associations. For example, you want to load all the articles that have
a specific tag on them. For more details on the `matching()`, look at
[Filtering By Associated Data](#filtering-by-associated-data).

> [!NOTE]
> MongoDB has no SQL-style `JOIN`. The join functions above compile to
> aggregation `$lookup` stages. To filter or load data through associations,
> prefer `contain()` and `matching()` over hand-built `$lookup` joins.

<a id="eager-loading-associations"></a>

## Eager Loading Associations Via Contain

By default, the ODM does not load **any** associated data when using `find()`.
...
surrounding lazy-loading in an ODM. In the ODM you state which associations
should be eager loaded using the 'contain' method:
...
function that receives as the first argument a query object, `\Crustum\Mongo\ODM\Query\SelectQuery`:
...
> [!WARNING]
> If the results are missing association documents, make sure the foreign key
> fields are selected in the query. Without the foreign keys, the ODM cannot
> find matching rows.
...
See [Associations - Linking Collections Together](./associations) for details on the default value and options for each association
type.
...
extremely simple to do with the ODM:
...
> [!NOTE]
> As this function will create an `INNER $lookup`, you might want to consider
...
`_matchingData` property of documents. If both match and contain the same
...
`INNER $lookup` that `matching()` uses with `innerJoinWith()`:
...
> ```
>
> The aliased field lands on the root document (verified by
> `testInnerJoinWithAliasedRootSelect`); without the alias the field is exposed
> under `_matchingData.<Alias>`.

> [!WARNING]
> You should not combine `innerJoinWith()` and `matching()` with the same association.
> This will produce multiple `$lookup` stages and might not create the query you
> expected.
...
> [!NOTE]
> As `notMatching()` will create a `LEFT $lookup`, you might want to consider
...
As mentioned earlier, you can customize the `strategy`
used by an association in a `contain()`.

For `BelongsTo` and `HasOne` associations the default strategy is `lookup`,
which loads the association in a `$lookup` stage inside the main query. It can
be changed to `select`:
...
The `select` strategy loads the associated data with a separate batched query
(`whereIn`) and matches the results in memory. This can be useful when you need
to add conditions that don't work well in a `$lookup`, or to avoid growing the
main pipeline.

Usually, you set the strategy for an association when defining it
in `BaseCollection::initialize()`, but you can permanently change the strategy
manually:

```php
$articles->getAssociation('Comments')->setStrategy('select');
```

> [!NOTE]
> The `join` and `subquery` strategy names are accepted as cake-compatibility
> aliases: `join` maps to the `lookup` strategy and `subquery` maps to the
> separate-query (`select`) loading path. See
> [Associations - Linking Collections Together](./associations) for the full
> strategy reference, including the ODM-native `embed` strategy for embedded
> documents.

### Fetching With The Separate-Query Strategy

As your collections grow in size, fetching associations from them can become
slower, especially if you are querying big batches at once. For `hasMany` and
`belongsToMany` associations the ODM already uses a separate batched query by
default (the cake `subquery` strategy name is mapped onto it), so large result
chunks are fetched without nesting the association load into the primary
pipeline:
...
The result will remain the same as with the default strategy. For
`belongsToMany` associations the matching/contain pipeline runs through the
junction collection, and `sort`/`fields`/`queryBuilder` options apply to the
loaded records as usual.

## Lazy Loading Associations

While the ODM uses eager loading to fetch your associations, there may be cases
where you need to lazy-load associations. You should refer to the
[Lazy Load Associations](./documents#lazy-load-associations) and [Loading Additional Associations](#loading-additional-associations)
sections for more information.

## Working with Result Sets

Once a query is executed with `all()`, you will get an instance of
`Crustum\Mongo\ODM\ResultSet`. This object offers powerful ways to manipulate
...
Result set objects will lazily load rows from the underlying Mongo cursor. By
default, results are buffered in memory allowing you to iterate a result
...
JSON respects hidden & virtual field settings on all document objects
within a result set.
...
done with result sets using the collections features. The [Format Results](./query-builder#format-results)
section show how you can add calculated fields, or replace the result set.

> [!WARNING]
> Mongo cursors are single-pass iterators, so the ODM always buffers the
> result set in memory. When working with large data sets (especially when
> calling collection methods like `extract()` on the result set), you may
> encounter high memory usage due to the entire result set being buffered.
>
> To reduce memory pressure, skip hydration so rows are returned as plain
> arrays instead of document objects:
>
> ``` bash
...
> ```
>
> See [Getting Arrays Instead of Documents](#getting-arrays-instead-of-entities).
...
as the first argument a query object, `\Crustum\Mongo\ODM\Query\SelectQuery`:

```php
$user = $this->Users->get($id);
$withMore = $this->Users->loadInto($user, ['Posts' => function (SelectQuery $query) {
    return $query->where(['Posts.status' => 'published']);
}]);
```

You can eager load additional data into a single document, or a collection of
documents.
...
found in the database. While documents' getter methods can take care of most of
...
a `friends` collection and you want to find "fake friends" in our database, or

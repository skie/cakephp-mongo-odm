...
/**
 * ORM proxy for the Direction-1 `BelongsTo` bridge association.
 *
 * Registers on `Table->associations()` so `contain('Authors')`, entity
 * property access, and `__call` forwarding work like a native Cake
 * BelongsTo while the load is a batched Mongo query.
 *
 * @extends \Cake\ORM\Association\BelongsTo<\Cake\ORM\Table>
 * @see docs/reference/29-orm-mongo-association-bridge.md §4
 */
class BelongsToProxy extends BelongsTo
{
    use ProxyTrait;

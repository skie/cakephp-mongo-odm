...
/**
 * ORM proxy for the Direction-1 `BelongsToMany` bridge association.
 *
 * Registers on `Table->associations()` so `contain('Tags')`, entity property
 * access, and `__call` forwarding work like a native Cake BelongsToMany while
 * the load is a batched Mongo query (junction or array pivot).
 *
 * @extends \Cake\ORM\Association\BelongsToMany<\Cake\ORM\Table>
 * @see docs/reference/29-orm-mongo-association-bridge.md §4
 */
class BelongsToManyProxy extends BelongsToMany
{
    use ProxyTrait;

...
/**
 * ORM proxy for the Direction-1 `HasOne` bridge association.
 *
 * Registers on `Table->associations()` so `contain('Posts')`, entity property
 * access, and `__call` forwarding work like a native Cake HasOne while the
 * load is a batched Mongo query.
 *
 * @extends \Cake\ORM\Association\HasOne<\Cake\ORM\Table>
 * @see docs/reference/29-orm-mongo-association-bridge.md §4
 */
class HasOneProxy extends HasOne
{
    use ProxyTrait;
